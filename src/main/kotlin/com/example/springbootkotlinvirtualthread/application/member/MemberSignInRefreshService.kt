...
@Service
class MemberSignInRefreshService(
...
): MemberSignInRefreshUseCase {

    @Transactional(rollbackFor = [Throwable::class])
    override fun refreshSignIn(command: MemberSignInRefreshCommand.RefreshSignIn): AuthToken {
        try {
            ...
            val myMemberId = authToken.memberId
            ...
            // 새로운 토큰 생성
            val newAccessToken =
                authenticationTokenManager.generateToken(authToken.memberId, AuthenticationTokenType.ACCESS)
            val newRefreshToken =
                authenticationTokenManager.generateToken(authToken.memberId, AuthenticationTokenType.REFRESH)
